...
/**
 * Interface for general Data Access Object that can be used for any type domain object. A single instance implementing
 * this interface can be used for multiple types of domain objects.
 * 
 * @author dwolverton
 */
public interface GeneralDAO {
	...
	/**
	 * Get all entities of the specified type from the datastore that have one of these ids. An array of entities
	 * is returned that matches the same order of the ids listed in the call. For each entity that is
	 * not found in the datastore, a null will be inserted in its place in the return array.
	 */
	 ...
	 * 
	 * <p>
	 * This does not require a call to the datastore and does not populate any of the entity's values. Values may be
	 * fetched lazily at a later time. This increases performance if a another entity is being saved that should
	...
	/**
	 * <p>
	 * Get a reference to the entities of the specified type with the given ids from the datastore. An array of entities
	 * is returned that matches the same order of the ids listed in the call.
	 * 
	 * <p>
	 * This does not require a call to the datastore and does not populate any of the entities' values. Values may be
	 * fetched lazily at a later time. This increases performance if a another entity is being saved that should
	...
	/**
	 * Search for objects given the search parameters in the specified <code>Search</code> object.
	 */
	...
	/**
	 * Returns the total number of results that would be returned using the given <code>Search</code> if there were no
